...
bool canFormDominoChainRec(Vector<Domino> & remaining, int designated){
    ...
    for (int i = 0; i < size; i++){
        int left = remaining[i].getLeftDots(), right = remaining[i].getRightDots();
        Vector<Domino> assist = remaining;
        assist.remove(i);
        if (left == designated)
            if (canFormDominoChainRec(assist, right))
                return true;
        if (right == designated)
            if (canFormDominoChainRec(assist, left))
                return true;
    }
    return false;
}
