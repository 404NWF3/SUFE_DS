...
int listAllFifteensRec(int target, Vector<Card> cards);
...
int listAllFifteens(Vector<Card> & cards) {
    return listAllFifteensRec(TARGET, cards);
}
...
int listAllFifteensRec(int target, Vector<Card> cards){
    ...
    res += listAllFifteensRec(target, cards.subList(1));
    if (top_rank == target) res++;
    if (top_rank < target)
        res += listAllFifteensRec(target - top_rank, cards.subList(1));
    return res;
}
